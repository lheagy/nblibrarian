...
SUPPORTED_SOURCES = ['github']  #: List of source-code hosts that are currently supported by this package
EXPECTED_CONFIG = [
    'library-config', 'nblibrary-config', 'library_config', 'nblibrary_config',
    'library', 'nblibrary'
...
class Librarian(object):
    ...
    def config_file(self):
        ...
        if getattr(self, '_config_file', None) is None:
            files = [f for f in os.listdir() if any([f.endswith(ext) for ext in EXPECTED_CONFIG_EXTENSION])]
    ...
    def notebook_list(self):
        if getattr(self, '_notebook_list', None) is None:
            ...
            if 'directory' in self.source.keys():
                of_interest = [c for c in content["tree"] if c['path'].startswith(self.source['directory'])]
            else:
